...
@mcp.tool()
async def ob_context(query: str = "") -> str:
    """获取当前请求所需的最小上下文：State、Active Plans 和相关 Memory。只读，不修改记忆或状态。"""
    pack = await context_service.get_pack(query=query)
    return json.dumps(pack.to_dict(), ensure_ascii=False)


@mcp.tool()
async def ob_gateway(query: str = "") -> str:
    """统一获取当前请求所需的 Context + Wake 状态。只读，不触发模型、不修改记忆或状态。"""
    wake = await WakeContextBuilder(context_service, bucket_mgr=bucket_mgr).check(query=query)
    return json.dumps({
        "kind": "ob_gateway",
        "version": "gateway.v1",
        ...
        "wake": {
            ...
            "signal": wake["wake_signal"],
        },
    }, ensure_ascii=False)

@mcp.tool()
async def ob_wake_context(query: str = "") -> str:
    """获取一次 Wake Context 快照。只读，不触发模型、不修改记忆或状态。"""
    wake = await WakeContextBuilder(context_service).build(query=query)
    return json.dumps(wake, ensure_ascii=False)
@mcp.tool()
async def ob_wake_check(query: str = "") -> str:
    """检查当前是否存在值得唤醒的信号。只读，不触发模型、不修改记忆或状态。"""
    wake = await WakeContextBuilder(context_service, bucket_mgr=bucket_mgr).check(query=query)
    return json.dumps(wake, ensure_ascii=False)
